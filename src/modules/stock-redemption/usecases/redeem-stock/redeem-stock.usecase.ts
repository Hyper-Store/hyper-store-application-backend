...
export class RedeemStockUsecase {
    ...
    async execute({ userId, signatureId }: RedeemStockUsecase.Input) {
        return await this.prismaClient.$transaction(async (prisma: PrismaClient) => {
            ...
            const redemptionCount = await prismaStockRedemptionRepository.getTodayRedemptionCount(userId, signatureId)
            ...
            const stockRedeemedEvent = new StockRedeemedEvent({
                ...stockRedemptionEntity.toJSON(),
                value: stock.value
            })
